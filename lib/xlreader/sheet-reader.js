...
function sheetToJson(sheet, opts){
    ...
    const { formatters, keepEmptyRows, skipIfFirstColumnEmpty, firstRowCommentChar } = opts || {};
    ...
    for (let r = range.s.r + 1; r <= range.e.r; r++) {
        const rowRef = XLSX.utils.encode_row(r);
        const row    = {};
        let isEmpty  = true;
        let firstColEmpty = false;

        for (let c = range.s.c; c <= range.e.c; c++) {
            const column    = columns[c - range.s.c];
            const fieldName = column.name;
            const cell      = sheet[column.ref + rowRef];
            if(cell === undefined || cell.t === undefined || cell.v === undefined) {
                if (c === range.s.c) firstColEmpty = true;
                row[fieldName] = column.default; // add but don't trigger isempty
                continue;
            }
            ...
            row[fieldName] = column.converter
