...
class ExcelFilePipeline extends EventEmitter {
    /**
     ...
     */
    constructor({destRoot, fileParser, mockProcessor, withHashing}) {
        super();
    ...
    async processFile(filepath) {
        ...
        try {
            const { blob, hash } = await this._readSourceFile(filepath);
            ...
            return {
                mockList: mockResults.map(r => r.relativeMockName),
                sourceFileHash: hash,
            };
    ...
    async _saveResultAsync(targetDirPath, mockFilename, data, rowCount) {
        ...
        this.emit('mock-processed', { name: mockFilename, rowCount });
    }
    ...
    async _readSourceFile(filepath) {
        ...
        if (this.withHashing) {
            ...
            hashObj.update(blob);
            hash = hashObj.digest().toString('hex');
        }
...
module.exports = ExcelFilePipeline;
